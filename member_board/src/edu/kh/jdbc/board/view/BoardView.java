...
public class BoardView {
	...
	public void boardMenu(Member loginMember) {
		...
		do {
			try {
				...
				e.printStackTrace();
				System.out.println("입력 형식이 올바르지 않습니다. 다시시도 해주세요");
	...
	private void updateBoard(int boardNo) {
		...
		try {
			...
			if(result > 0 ) {
				System.out.println(boardNo + "번 게시글을 수정하였습니다.");
			}else {
	...
	private String inputContent() {
		...
		while(true) { // @exit가 입력될 때 까지 무한히 문자열을 입력 받아 하나의 변수에 누적 
			// == 게시글 내용
	...
	private void updateReply(int replyNo) {
		System.out.print("수정할 내용 입력(종료 시 @exit 입력) : ");
		...
		try {
			...
			if(result > 0) System.out.println(replyNo + "\n번 댓글이 수정되었습니다\n");
